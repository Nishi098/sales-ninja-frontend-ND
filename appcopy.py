...
df['Month'] = pd.to_datetime(df['DateKey']).dt.month
selected_month = selected_date_dt.month


actuals_monthly_df = df[(df['Month'] < selected_month) & (df['Year'] == selected_year)]
...
last_day_last_month_of_year = pd.Timestamp(
    year=selected_date_dt.year,
    month=12,
    day=31
)
...
last_day_last_month_of_year = last_day_last_month_of_year.strftime("%Y-%m-%d")

# Construct URL with query parameters
month_url = f"https://salesninjaapi-752034082007.europe-west1.run.app/predict_basic?min_date={first_day_current_month}&max_date={last_day_last_month_of_year}"
api_data_month = requests.get(month_url).json()
api_month_df = pd.DataFrame()
api_month_df['DateKey'] = api_data_month.keys()
api_month_df['DateKey'] = pd.to_datetime(api_month_df['DateKey'])
api_month_df['Forecast'] = api_data_month.values()
api_month_df['Forecast'] = api_month_df['Forecast'] / 1000

forecast_monthly_df = api_month_df[(api_month_df['DateKey'] >= first_day_current_month) & (api_month_df['DateKey'] <= last_day_last_month_of_year)]



#v. Plotting the monthly development graph
merged_monthly_df = actuals_monthly_df[['DateKey', 'SalesAmount']].merge(
    forecast_monthly_df, on='DateKey', how='outer'
)
grouped_monthly_df = merged_df.groupby('DateKey', as_index=False)[['SalesAmount', 'Forecast']].sum()


# Ensure DateKey is datetime and format as date string
# Make sure 'DateKey' is datetime
grouped_monthly_df['DateKey'] = pd.to_datetime(grouped_monthly_df['DateKey'])

# Filter for the selected year
monthly_data = grouped_monthly_df[grouped_monthly_df['DateKey'].dt.year == selected_year]

# Group by month and sum (if you have multiple entries per month)
monthly_sums = monthly_data.groupby(monthly_data['DateKey'].dt.month, as_index=False)[['SalesAmount', 'Forecast']].sum()



monthly_sums['MonthName'] = monthly_sums['DateKey'].apply(lambda x: calendar.month_abbr[x])
# If 'DateKey' was replaced by the groupby result, use:
monthly_sums['MonthName'] = monthly_sums['Month'].apply(lambda x: calendar.month_abbr[x])
# (Adjust 'Month' or 'DateKey' as needed—if groupby uses 'DateKey', the above may not work; see below fix)


monthly_sums = monthly_data.groupby(monthly_data['DateKey'].dt.month, as_index=False)[['SalesAmount', 'Forecast']].sum()
monthly_sums.rename(columns={'DateKey': 'Month'}, inplace=True)
monthly_sums['MonthName'] = monthly_sums['Month'].apply(lambda x: calendar.month_abbr[x])

fig, ax = plt.subplots(figsize=(10, 6))

bar_width = 0.4
x = range(len(monthly_sums))

ax.bar(
    [i - bar_width/2 for i in x],
    monthly_sums['SalesAmount'],
    width=bar_width,
    label='SalesAmount',
    color='#FF1493'
)
...
ax.set_xticks(x)
ax.set_xticklabels(monthly_sums['MonthName'], rotation=45, ha='right')
ax.set_xlabel('Month')
ax.set_ylabel('Sum')
ax.legend()
